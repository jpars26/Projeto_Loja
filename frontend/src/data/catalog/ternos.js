// Catálogo da categoria "ternos". Ids de coleção usam sempre o prefixo
// "ternos-" — nunca usar um id igual ao literal de categoria ("noivas",
// "ternos", "festa"), pois a rota /collections/:slug distingue categoria de
// id de coleção checando esse literal primeiro (ver App.js).
//
// Produtos com fotos placeholder até termos as fotos reais da coleção.
import img_1_assets_images_placeholders_foto_em_breve from "../../assets/images/placeholders/foto-em-breve.svg";

const placeholder = img_1_assets_images_placeholders_foto_em_breve;

const ternos = [
  {
    id: "ternos-classico",
    name: "Ternos - Coleção Clássica",
    image: placeholder,
    banner: placeholder,
    category: "ternos",
    products: [
      {
        id: "ternos-classico-p1",
        name: "Terno Slim Azul-Marinho",
        image: placeholder,
        model: "terno",
        color: "azul-marinho",
      },
      {
        id: "ternos-classico-p2",
        name: "Terno Risca de Giz Cinza",
        image: placeholder,
        model: "terno",
        color: "cinza-claro",
      },
      {
        id: "ternos-classico-p3",
        name: "Terno Preto Cerimônia",
        image: placeholder,
        model: "terno",
        color: "preto",
      },
    ],
  },
];

export default ternos;
